...
def run_contract_review_task(
    ...
    # ── Poison-pill guard ─────────────────────────────────────────────────
    # task_acks_late + task_reject_on_worker_lost re-queues on crash WITHOUT
    # incrementing self.request.retries. Guard against infinite crash-loops
    # by maintaining a separate Redis run counter.
    RUN_CAP = 3
    run_key = f"celery:run_count:{contract_id}"

    async def _check_run_count() -> int:
        r = aioredis.from_url(config.CELERY_BROKER_URL, encoding="utf-8", decode_responses=True)
        try:
            count = await r.incr(run_key)
            await r.expire(run_key, 86400)  # 1-day TTL
            return int(count)
        finally:
            await r.aclose()

    try:
        run_count = asyncio.run(_check_run_count())
    ...
    if run_count > RUN_CAP:
        logger.error(
            ...
            contract_id, RUN_CAP, run_count,
        )
        return {"contract_id": contract_id, "status": "failed", "error": "Maximum run attempts exceeded."}
    # ─────────────────────────────────────────────────────────────────────
