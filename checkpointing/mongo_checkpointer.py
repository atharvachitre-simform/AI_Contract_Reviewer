...
class MongoCheckpointerStore:
    ...
    _shared_client: MongoClient | None = None
    ...
    def __init__(self, uri: str | None = None):
        ...
        if self.uri:
            try:
                if MongoCheckpointerStore._shared_client is None:
                    MongoCheckpointerStore._shared_client = MongoClient(
                        self.uri,
                        serverSelectionTimeoutMS=5000,
                        maxPoolSize=50,
                        minPoolSize=5,
                    )
                    MongoCheckpointerStore._shared_client.admin.command("ping")
                    logger.info("MongoDB checkpointer connected successfully.")
                self.client = MongoCheckpointerStore._shared_client
