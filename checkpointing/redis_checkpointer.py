...
class RedisCheckpointer:
    ...
    async def verify_or_update_hash(self, contract_text: str) -> bool:
        ...
        # If not in local fallback, try MongoDB
        if not stored_hash and self._mongo.is_connected():
            try:
                meta_doc = self._mongo.load_checkpoint(self.contract_id, "metadata")
                if meta_doc:
                    stored_hash = meta_doc.get("contract_text_hash")
            except Exception:
                pass
        ...
        if not stored_hash:
            ...
            if self._mongo.is_connected():
                try:
                    self._mongo.save_checkpoint(
                        self.contract_id, "metadata", {"contract_text_hash": new_hash}
                    )
                except Exception:
                    pass
            return True
        ...
        if self._mongo.is_connected():
            try:
                self._mongo.delete_checkpoints(self.contract_id, "metadata")
            except Exception:
                pass
