...
from src.helpers.llm_parsing import parse_llm_json_response, parse_llm_response
from src.helpers.obligation_heuristics import (
...
from src.prompts.obligation_finder_prompt import (
    OUTPUT_SCHEMA,
    SYSTEM_INSTRUCTION,
...
class ObligationFinderAgent:
    ...
    def _refinement_node(self, state: ObligationFinderState) -> ObligationFinderState:
        ...
        logger.info("_refinement_node: starting obligation refinement for %d items", len(state.get("obligations") or []))
        refined_obligations = []
        ...
        logger.info("Merged similar obligations. Size went from %d to %d", old_count, len(refined_obligations))
        ...
        logger.info("_refinement_node: completed refinement. payment: %d, notice: %d, restriction: %d, general: %d. Deadlines count: %d", len(categorized["payment"]), len(categorized["notice"]), len(categorized["restriction"]), len(categorized["general"]), len(key_deadlines))
        return state
    ...
    def _get_missed_clauses(self, state: ObligationFinderState) -> list[Any]:
        ...
        logger.info("_get_missed_clauses: completed scan. Found %d missed clauses.", len(missed_clauses))
