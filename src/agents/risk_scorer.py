...
from src.models import ClauseExtractorOutput, RiskIssue, RiskLevel, RiskScorerOutput
...
class RiskScorerAgent:
    ...
    def _retrieve_reference_risks_node(self, state: RiskScorerState, retriever: Any | None) -> RiskScorerState:
        ...
        try:
            ...
            reference_risks = retrieve_from_knowledge_base(AzureClientFactory(), query, "legal_standards")
    ...
    def _normalize_risk_level(self, raw_value: str | None) -> RiskLevel:
        ...
        if norm == RiskLevel.MEDIUM and raw_value and raw_value.strip().lower() not in {"medium", "m", "moderate"}:
            return RiskLevel.LOW
        return norm
