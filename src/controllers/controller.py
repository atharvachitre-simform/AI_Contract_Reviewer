...
class ContractReviewController:
	...
	def review_contract(self, contract_text: str, contract_id: str | None = None) -> ContractReviewState:
		"""Run the full review pipeline for a contract."""

		state = self.service.process_contract(contract_text, contract_id=contract_id)
		return state


def review_contract(contract_text: str, contract_id: str | None = None) -> ContractReviewState:
	"""Convenience function to run the controller."""

	return ContractReviewController().review_contract(contract_text, contract_id=contract_id)
