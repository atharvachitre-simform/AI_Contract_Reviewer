...
class ContractReviewWorkflow:
    ...
    def _run_clause_extraction(
        ...
        logger.info("Completed clause extraction agent. Extracted %d clauses.", len(clause_extraction.clauses))
    ...
    def _run_parallel_analysis(
        ...
        def _worker_initializer(
            tid: str, u_id: str | None, s_id: str | None, c_id: str | None
        ) -> None:
        ...
        try:
            with ThreadPoolExecutor(
                ...
                logger.info("Completed obligation finder agent. Found %d obligations.", len(state.obligation_finding.obligations))
                ...
                logger.info("Completed red flag detector agent. Found %d red flags.", len(state.red_flag_detection.red_flags))
                ...
                logger.info("Completed risk scorer agent. Found %d issues, overall risk level: %s", len(state.risk_scoring.issues), state.risk_scoring.overall_risk_level)
        except Exception as e:
    ...
    def _run_sequential_analysis(
        ...
        logger.info("Completed plain English writer agent. Generated %d summaries.", len(state.plain_english.clause_summaries))

        logger.info("Running report assembler agent")
        try:
            assert state.clause_extraction is not None
            assert state.risk_scoring is not None
            assert state.red_flag_detection is not None
        ...
        self._trace(
            ...
            "Completed report assembly.",
            {
                ...
                "risk": str(state.final_report.overall_risk_level) if state.final_report else "None",
            },
            "completed",
            trace_id=trace_id,
        )
    ...
    def run(
        ...
        *,
        ...
        logger.info("Contract review workflow completed successfully for contract_id: %s", state.contract_id)
        self.tracer.flush()
        return state
...
def run_contract_review(
    ...
    *,
    ...
    logger.info("run_contract_review: entry for contract_id=%s, source_file=%s", contract_id, source_file)
