...
class AsyncContractReviewWorkflow:
    ...
    async def _run_in_executor(fn: Callable[..., Any], *args: Any, **kwargs: Any) -> Any:
        ...
        def wrapper() -> Any:
            ...
            try:
                ...
                logger.error("Executor thread failed running %s: %s", fn_name, str(e), exc_info=True)
                raise e
    ...
    async def run(
        ...
        *,
        ...
        try:
            ...
            logger.error("AsyncContractReviewWorkflow.run: failed during streaming: %s", str(e), exc_info=True)
            raise e
        ...
        *,
        ...
        try:
            async for event in self._stream_clause_extraction(
                state, completed, checkpointer, contract_text, source_file, llm_client, memory_context, retriever
            ):
                yield event
        except Exception:
            return
        ...
        async for event in self._stream_parallel_analysis(
            state, completed, checkpointer, filtered_extraction,
            obligation_llm_client, red_flag_llm_client, risk_llm_client,
            retriever, memory_context, perspective
        ):
            yield event
    ...
    async def _check_and_load_step(
        self, state: ContractReviewState, checkpointer: Any, completed: set[str], step: str, output_cls: Any
    ) -> bool:
    ...
    async def _stream_parallel_analysis(
        ...
        obl_skipped = await self._check_and_load_step(state, checkpointer, completed, obligation_step, ObligationFinderOutput)
        red_skipped = await self._check_and_load_step(state, checkpointer, completed, red_flag_step, RedFlagDetectorOutput)
        risk_skipped = await self._check_and_load_step(state, checkpointer, completed, risk_step, RiskScorerOutput)
        ...
        if not obl_skipped:
            ...
            tasks_to_run.append(("obligation", self._run_in_executor(find_obligations, filtered_extraction, obligation_llm_client, memory_context, perspective)))
        if not red_skipped:
            yield {"step": red_flag_step, "status": "started", "detail": {}}
            tasks_to_run.append(("red_flag", self._run_in_executor(detect_red_flags, filtered_extraction, red_flag_llm_client, perspective)))
        if not risk_skipped:
            yield {"step": risk_step, "status": "started", "detail": {}}
            tasks_to_run.append(("risk", self._run_in_executor(score_risks, filtered_extraction, risk_llm_client, retriever, memory_context, perspective)))
        ...
        if tasks_to_run:
            ...
            for (label, _), result in zip(tasks_to_run, results):
                async for event in self._process_parallel_result(label, result, state, checkpointer):
                    yield event
