...
from src.helpers.extraction_tracer import get_tracer as _get_tracer
from src.helpers.pdf_cleaner import preprocess_for_extraction
from src.helpers.tracing import trace_step
from src.models import ClauseExtractorOutput, ContractReviewState, ProcessingStatus
...
from src.services.memory_service import load_checkpoint, persist_results, save_checkpoint
from src.services.memory_store import MemoryStore
from src.services.redis_client import AsyncRedisClient
from src.services.retrieval_service import extract_from_pdf, retrieve_from_knowledge_base
from src.workflows.workflow import ContractReviewWorkflow, run_contract_review
...
class ContractReviewService:
    ...
    def _trace(
        ...
        trace_step(self.tracer, step, description, payload, status, trace_id or self.current_trace_id)
    ...
    def _save_memory(
        self, contract_id: str | None, session_id: str, results: ContractReviewState
    ) -> None:
        ...
        if contract_id:
            logger.debug("Saving long term memory and indexing clauses for contract_id=%s", contract_id)
            long_term_payload = {
                "contract_id": contract_id,
                "perspective": results.perspective,
                "review_summary": (
            ...
            if results.clause_extraction and results.clause_extraction.clauses:
                logger.debug("Indexing %d clauses in Qdrant for contract_id=%s", len(results.clause_extraction.clauses), contract_id)
    ...
    async def get_bulk_review_status(self, batch_id: str) -> dict[str, Any]:
        ...
        if not mapping_str:
            logger.warning("Bulk review status check failed: batch_id=%s not found in Redis", batch_id)
        ...
        logger.debug("Checked status info for batch_id=%s: status=%s", batch_id, status_info.get("status"))
        ...
        if status_info["status"] == "completed":
            ...
            logger.info("Batch %s completed. Processing downstream tasks for %d contracts", batch_id, len(contracts_map))
            ...
            async def process_downstream(c_id: str, extracted_data: ClauseExtractorOutput) -> None:
                ...
                def _run_workflow() -> ContractReviewState:
                    ...
                    try:
                        ...
                        logger.debug("Obligation finding and red flag detection futures resolved for contract_id=%s", c_id)
                        ...
                        logger.info("process_downstream: completed workflow successfully for contract_id=%s", c_id)
                        return state
                    except Exception as e:
                        logger.error("Error during downstream workflow processing for contract_id=%s: %s", c_id, str(e), exc_info=True)
