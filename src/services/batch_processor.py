...
from ..helpers.llm_parsing import parse_llm_response
...
class BatchProcessor:
    ...
    def compile_extraction_batch(self, contracts: list[dict[str, str]]) -> str:
        ...
        model_name = os.getenv("AZURE_OPENAI_DEPLOYMENT_CLAUSE_EXTRACTOR", config.DEFAULT_BATCH_MODEL)
        ...
        try:
            for contract in contracts:
                ...
                for idx, unit in enumerate(units):
                    ...
                    if (
                        relevance_score < MIN_RELEVANCE_THRESHOLD
                        and not contains_risk_trigger_terms(unit["text"])
                    ):
                        continue
