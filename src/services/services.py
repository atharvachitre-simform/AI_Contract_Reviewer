...
from src.services.azure_clients import AzureClientFactory, MemoryStore
...
from src.workflows.workflow import ContractReviewWorkflow
...
from src.helpers.pdf_cleaner import preprocess_for_extraction, clean_extracted_pages
from src.helpers.extraction_tracer import get_tracer as _get_tracer
from src.workflows.workflow import run_contract_review
...
class ContractReviewService:
    ...
    def _save_memory(self, contract_id: str | None, session_id: str, results: ContractReviewState) -> None:
        short_term_payload = {
            ...
            "negotiation_priorities": [{"title": negotiation_priority.title, "priority": negotiation_priority.priority, "reason": negotiation_priority.reason} for negotiation_priority in results.final_report.negotiation_priorities] if results.final_report else [],
        }
        ...
        if contract_id:
            long_term_payload = {
                ...
                "missing_clauses": [{"category": missing_clause.category, "reason": missing_clause.reason} for missing_clause in results.final_report.missing_clauses] if results.final_report else [],
                "negotiation_priorities": [{"title": negotiation_priority.title, "priority": negotiation_priority.priority, "reason": negotiation_priority.reason} for negotiation_priority in results.final_report.negotiation_priorities] if results.final_report else [],
            }
