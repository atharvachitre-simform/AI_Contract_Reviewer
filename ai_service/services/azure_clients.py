...
from ai_service.utils.pdf_cleaner import clean_extracted_pages, clean_extracted_paragraphs
...
from .llm_client import AzureOpenAIWrapper
...
class AzureClientFactory:
    ...
    def _init_qdrant_with_retry(self, qdrant_url: str, api_key: str | None) -> Any:
        import time
        last_err = None
        for attempt in range(1, 4):
            try:
                return QdrantClient(url=qdrant_url, api_key=api_key, timeout=10.0)
            except Exception as err:
                last_err = err
                logger.warning(
                    f"QdrantClient initialization attempt {attempt}/3 failed: {err}. Retrying..."
                )
                if attempt < 3:
                    time.sleep(2 ** (attempt - 1))  # 1s, 2s, 4s delays
        raise last_err
    ...
    def _init_qdrant_client(self) -> Any | None:
        ...
        if not qdrant_url:
            return None
        try:
            return self._init_qdrant_with_retry(qdrant_url, qdrant_api_key)
        except Exception as e:
            logger.error(f"Qdrant client initialization failed after 3 attempts: {e}")
            return None
    ...
    def search_documents(
        ...
        from ai_service.services.search_client import search_documents as sd
        return sd(self, query, index_name, top_k)
