...
from ai_service.services.content_filter import (
...
from ai_service.prompts.system_context import BUSINESS_DOMAIN_HEADER, DEFAULT_AGENT_SYSTEM_PROMPT  # noqa: E402
...
class AzureOpenAIWrapper:
    ...
    def get_embedding(self, text: str) -> list[float]:
        ...
        if self.azure_client is not None:
            ...
            return list(response.data[0].embedding)  # type: ignore
    ...
    def chat_complete(
        ...
        Returns:
            str | None: The completion response text, or None if the request fails or is blocked.
        """
        try:
            res: str | None = ""
            raw_response = None
            ...
            from ai_service.utils.telemetry import log_chat_complete_telemetry
            log_chat_complete_telemetry(
                getattr(self, "agent_name", "chat_complete"),
            ...
            from ai_service.utils.telemetry import log_chat_complete_telemetry
            log_chat_complete_telemetry(
                getattr(self, "agent_name", "chat_complete_failed"),
                self.deployment_name,
                prompt,
                system_prompt,
                "",
    ...
    def _call_azure(
        ...
        if response_format is not None:
            kwargs["response_format"] = response_format
        assert self.azure_client is not None
        try:
    ...
    def _execute_chat_complete(
        ...
        if system_prompt:
            ...
            sys_content = DEFAULT_AGENT_SYSTEM_PROMPT
        ...
        try:
            ...
            if self._is_rate_limit_error(e) and config.GROQ_API_KEY and not self.use_groq:
                ...
                if fallback_wrapper.is_configured():
                    ...
                    return res or "", self._last_response
            raise
    ...
    def chat_complete_multimodal(
        ...
        try:
            ...
            try:
                self._last_response = None
                res, raw_response = self._execute_chat_complete_multimodal(messages, max_tokens, temperature)
            except Exception as e:
                if is_content_filter_error(e):
                    logger.warning(
