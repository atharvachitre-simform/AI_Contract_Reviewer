...
def clean_extracted_pages(pages: list[str]) -> str:
    ...
    if noise_lines:
        logger.info(
            f"Dynamically detected and stripping {len(noise_lines)} repeated header/footer lines: {noise_lines}"
        )
...
def clean_extracted_paragraphs(paragraphs: list[str]) -> list[str]:
    ...
    noise_paragraphs = {
        p
        for p, count in paragraph_counter.items()
...
# Uses a plain (non-DOTALL) pattern so it only captures text on the same
# logical line; the actual truncation is applied conditionally in
# preprocess_for_extraction only when the phrase appears in the last 25%.
...
# Stops before the first numbered clause or ARTICLE/SECTION heading so that
# operative body text immediately after WHEREAS is never consumed.
...
def preprocess_for_extraction(text: str) -> tuple[str, dict]:
    ...
    # 1. Truncate signature block — only if the phrase appears in the last 25%
    #    of the document to avoid stripping body text that references this phrase.
    sig_match = _SIGNATURE_BLOCK.search(text)
    if sig_match and sig_match.start() >= len(text) * 0.75:
