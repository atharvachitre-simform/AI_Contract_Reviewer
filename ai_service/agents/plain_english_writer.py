...
from ai_service.output_schemas.models import ClauseExtractorOutput, PlainEnglishClause, PlainEnglishWriterOutput
from ai_service.prompts.plain_english_writer_prompt import (
    build_plain_english_writer_prompt,
    ...
    try:
        ...
        if all_clause_summaries or all_exec_summaries:
            if len(all_exec_summaries) > 1:
                ...
                synthesis_prompt = build_synthesis_prompt(summaries_to_merge)
                response_text = run_agent_tool_loop(
                    llm_client=llm_client,
...
def validate_summaries_node(state: PlainEnglishWriterState) -> PlainEnglishWriterState:
    ...
    if not state["llm_attempt_success"] or not state["clause_summaries"]:
        logger.warning(
            "validate_summaries_node: LLM summary generation failed or returned empty. Using fallback generation."
        )
        ...
        if clauses:
            ...
            state["plain_english_risk_notes"] = []
        logger.info(
            "validate_summaries_node: Generated %d fallback summaries",
            len(state["clause_summaries"]),
        )
    else:
        logger.info(
            "validate_summaries_node: LLM generated summaries successfully. Count: %d",
            len(state["clause_summaries"]),
        )
    return state
