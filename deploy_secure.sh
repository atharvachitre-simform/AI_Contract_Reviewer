...
if [ -z "$GROQ_API_KEY" ] || [ -z "$QDRANT_API_KEY" ] || [ -z "$SUPABASE_PASSWORD" ]; then
    echo "ERROR: Missing required environment variables (GROQ_API_KEY, QDRANT_API_KEY, or SUPABASE_PASSWORD)."
    exit 1
fi
...
ASSIGNEE_TYPE=$(az account show --query user.type -o tsv)
if [ "$ASSIGNEE_TYPE" = "servicePrincipal" ]; then
    SP_CLIENT_ID=$(az account show --query user.name -o tsv)
    USER_OBJECT_ID=$(az ad sp show --id $SP_CLIENT_ID --query id -o tsv)
...
az role assignment create \
    ...
    --assignee-principal-type $ASSIGNEE_PRINCIPAL_TYPE || echo "Deployer role assignment already exists or could not be assigned."
