...
def mock_contract_env():
    ...
    mock_data = {
        "metadata": {
            ...
            "parties": [
                {"name": "Alpha Corp", "role": "alpha"},
                {"name": "Beta LLC", "role": "beta"},
            ],
            "agreement_date": "2026-01-01",
            "effective_date": "2026-01-10",
            "governing_law": "Delaware",
        },
        "risk_scorer": {"overall_risk_level": "medium", "overall_risk_score": 0.45},
        "report_assembler": {"verdict": "needs_amendment"},
        "obligation_finder": {
            "obligations": [
                {
                    "party": "Alpha Corp",
...
def test_tool_retrieve_contract_metadata(mock_contract_env):
    ...
    res = tool_retrieve_contract_metadata(mock_contract_env)
    data = json.loads(res)
...
def test_tool_list_active_obligations(mock_contract_env):
    ...
    res = tool_list_active_obligations(mock_contract_env)
...
def test_tool_fetch_page_visual_screenshot_success(mock_contract_env):
    ...
    res = tool_fetch_page_visual_screenshot(mock_contract_env, page_number=1)
...
def test_tool_fetch_page_visual_screenshot_missing(mock_contract_env):
    ...
    res = tool_fetch_page_visual_screenshot(mock_contract_env, page_number=99)
