...
def test_check_completeness_missing_signature():
    ...
    text = (
        "This is a contract of sale. The price is $100. The delivery is scheduled for Tuesday. "
        * 15
    )
...
def test_report_assembler_completeness_integration():
    ...
    agent = ReportAssemblerAgent(
        llm_client=None
    )  # llm_client is None, will trigger validate_report fallback

    # Incomplete contract text
    clause_output = ClauseExtractorOutput(
        raw_contract_text="This is a contract. We are missing signature block", clauses=[]
    )
    risk_output = RiskScorerOutput(
        overall_risk_level=RiskLevel.MEDIUM, overall_risk_score=0.5, issues=[]
    )
    red_flag_output = RedFlagDetectorOutput(red_flags=[], high_severity_count=0, summary="")
    plain_output = PlainEnglishWriterOutput(
        executive_summary="Summary", clause_summaries=[], key_points=[], plain_english_risk_notes=[]
    )
