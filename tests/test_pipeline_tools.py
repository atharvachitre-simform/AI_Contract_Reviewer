...
def test_verify_raw_text_existence():
    ...
    # Exact match
    res = execute_pipeline_tool("verify_raw_text_existence", {"snippet": "laws of California"}, context)
    assert "Verification SUCCESS" in res

    # Mismatch
    res = execute_pipeline_tool("verify_raw_text_existence", {"snippet": "laws of Delaware"}, context)
...
def test_date_calculator():
    ...
    # Days offset
    res = execute_pipeline_tool("date_calculator", {"base_date": "2026-06-12", "relative_term": "30 days after"}, context)
    assert "2026-07-12" in res

    # Months offset
    res = execute_pipeline_tool("date_calculator", {"base_date": "2026-06-12", "relative_term": "12 months following"}, context)
    assert "2027-06-07" in res  # 360 days approx

    # Years offset
    res = execute_pipeline_tool("date_calculator", {"base_date": "2026-06-12", "relative_term": "2 years from"}, context)
    assert "2028-06-11" in res  # 730 days approx

    # Error handling
    res = execute_pipeline_tool("date_calculator", {"base_date": "invalid", "relative_term": "30 days after"}, context)
    assert "Error" in res
...
def test_lookup_obligation_standards():
    ...
    context = {}
    res_saas = execute_pipeline_tool("lookup_obligation_standards", {"contract_type": "SaaS"}, context)
    assert "Net 30" in res_saas

    res_nda = execute_pipeline_tool("lookup_obligation_standards", {"contract_type": "NDA"}, context)
...
def test_query_compliance_playbook():
    ...
    res1 = execute_pipeline_tool("query_compliance_playbook", {"clause_type": "Indemnification", "text": "Vendor shall defend and indemnify Customer, but Customer has no obligation to indemnify Vendor."}, context)
    assert "Deviation Alert" in res1
    assert "Unilateral" in res1

    # Uncapped liability deviation
    res2 = execute_pipeline_tool("query_compliance_playbook", {"clause_type": "Limitation of Liability", "text": "Liability under this section is uncapped and unlimited."}, context)
    assert "Deviation Alert" in res2
    assert "Uncapped" in res2

    # Compliant clause
    res3 = execute_pipeline_tool("query_compliance_playbook", {"clause_type": "Governing Law", "text": "Delaware governs this agreement."}, context)
...
def test_search_legal_definitions():
    ...
    res_missing = execute_pipeline_tool("search_legal_definitions", {"concept": "unknown concept"}, context)
...
def test_retrieve_compliance_standards():
    ...
    context = {}
    res = execute_pipeline_tool("retrieve_compliance_standards", {"clause_type": "data transfer"}, context)
    assert "GDPR" in res

    res_privacy = execute_pipeline_tool("retrieve_compliance_standards", {"clause_type": "privacy"}, context)
...
def test_lookup_historical_score_rationale():
    ...
    context = {}
    res_unlimited = execute_pipeline_tool("lookup_historical_score_rationale", {"clause_type": "Limitation of Liability", "text": "unlimited liability"}, context)
    assert "CRITICAL risk" in res_unlimited

    res_mutual = execute_pipeline_tool("lookup_historical_score_rationale", {"clause_type": "Indemnification", "text": "mutual indemnification"}, context)
...
def test_jargon_translator():
    ...
    context = {}
    res = execute_pipeline_tool("jargon_translator", {"legalese": "Each party shall indemnify, defend, and hold harmless the other party."}, context)
...
def test_fetch_company_document_checklist():
    ...
    context = {}
    res_nda = execute_pipeline_tool("fetch_company_document_checklist", {"contract_type": "NDA"}, context)
...
def test_evaluate_negotiation_priority():
    ...
    # Critical risk
    res_crit = execute_pipeline_tool("evaluate_negotiation_priority", {"risk_score": 0.8, "red_flags": 3}, context)
    data_crit = json.loads(res_crit)
    assert data_crit["negotiation_priority"] == "CRITICAL"

    # High risk
    res_high = execute_pipeline_tool("evaluate_negotiation_priority", {"risk_score": 0.5, "red_flags": 1}, context)
    data_high = json.loads(res_high)
    assert data_high["negotiation_priority"] == "HIGH"

    # Low risk
    res_low = execute_pipeline_tool("evaluate_negotiation_priority", {"risk_score": 0.1, "red_flags": 0}, context)
